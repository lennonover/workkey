...
var spiderPage = {
    ...
    getCallbackData :function (err, res) {
        ...
        if (res.ok) {
            ...
            var arr = [],
                companyList = res.body.content.positionResult;
                //console.log(res.body.content.positionResult)
            if (companyList.length == 0) return false;
            //处理数据
            companyList.forEach(function (item,index) {
                console.log(item)
                var salary = item.position.salary.split('-');
                ...
                //过滤出所需数据
                arr[index] = {
                    companyFullName: item.position.companyFullName,
                    positionId : item.position.positionId ,
                    city:item.position.city ,
                    companySize:item.position.companySize ,
                    financeStage:item.position.financeStage ,
                    salary:aveSalary ,
                    workYear:item.position.workYear ,
